...
void SPUThread::process_mfc_cmd(u32 cmd)
{
	...
	LOG_ERROR(SPU, "Unknown command %s (cmd=0x%x, lsa=0x%x, ea=0x%llx, tag=0x%x, size=0x%x)",
...
bool SPUThread::set_ch_value(u32 ch, u32 value)
{
	...
	LOG_ERROR(SPU, "Unknown/illegal channel (ch=%d [%s], value=0x%x)", ch, ch < 128 ? spu_ch_name[ch] : "???", value);
}
